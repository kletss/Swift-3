...
//        deleteDatabase()
    ...
    func loadDatabase() {
        self.realtimeDB.child("Searche").getData { error, snapshot in
            if snapshot.exists()  {
        ...
        firestoreDB
            ...
            .getDocuments { snapshot, error in
                snapshot?.documents.forEach {
                    print("Document deleted \($0.reference.path)")
                    $0.reference.delete()
                }
            }
    }
    ...
    func saveDatabase() {
        realtimeDB
            .child("Searche")
            .setValue(["Group":searcheBar.text])
        ...
        firestoreDB
            ...
            .getDocuments { snapshot, error in
                snapshot?.documents.forEach { sn in
                    var fSn = false
                    self.filteredGroups.forEach { vkg in
                        if String(vkg.id) == sn.documentID {fSn = true}
                    }
                    if !fSn {sn.reference.delete()
                    print("Document deleted \(sn.reference.path)")
                    }
                }
            }
        
        if !filteredGroups.isEmpty {
            filteredGroups.forEach { vkg in
            firestoreDB
                .collection("Groups")
                .document(String(vkg.id))
                .setData(["ID":vkg.id, "name":vkg.name, "URL":vkg.photoAvatarURL], merge: false) { [weak self] error in
                    if let error = error {
                        self?.showAlert(
                            title: "Error",
                            message: error.localizedDescription)
                    } else {
                        print(" Data Saved \(vkg.id)")
                    }
                }
            }
        }
